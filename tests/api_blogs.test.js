...
describe('route put /:id', () => {
  ...
  test('changes only updated likes value to blog', async () => {
    ...
    let blogToUpdate = { ...helper.initialBlogs[0], likes: origLikes + addedAmountOfLikes }
    ...
    const blogsAtEnd = await helper.blogsInDb()
    expect({ ...blogsAtEnd[0] }).toEqual({ ...blogsAtStart[0], likes: (origLikes + addedAmountOfLikes) })
  })
})
